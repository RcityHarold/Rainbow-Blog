...
impl SubscriptionService {
    ...
    pub async fn create_subscription_plan(
        ...
        request
            .validate()
            .map_err(|e| AppError::Validation(format!("订阅计划数据验证失败: {}", e)))?;
        ...
        let (stripe_product_id, stripe_price_id) = self
            .stripe_service
            .create_plan_product_and_price(
                &plan_id,
                creator_id,
                &request.name,
        ...
        info!("Subscription plan created: {}", plan.id);
        Ok(plan)
    }
    ...
    pub async fn update_subscription_plan(
        ...
        debug!(
            "Updating subscription plan: {} for creator: {}",
            plan_id, creator_id
        );
        ...
        request
            .validate()
            .map_err(|e| AppError::Validation(format!("订阅计划更新数据验证失败: {}", e)))?;

        // 检查计划是否存在且属于该创作者
        self.verify_plan_ownership(plan_id, creator_id).await?;

        let existing_plan = self.get_subscription_plan(plan_id).await?;
        ...
        if let Some(price) = request.price {
            ...
            updates.push(("price".to_string(), json!(price)));
        }
        if let Some(benefits) = &request.benefits {
            updates.push(("benefits".to_string(), json!(benefits)));
        }
        if let Some(is_active) = request.is_active {
            updates.push(("is_active".to_string(), json!(is_active)));
        }

        let mut stripe_product_id = existing_plan.stripe_product_id.clone();
        let mut stripe_price_id = existing_plan.stripe_price_id.clone();
        let mut product_updated = false;
        let mut price_updated = false;

        let desired_name = request.name.clone().unwrap_or(existing_plan.name.clone());
        let desired_description = request
            .description
            .clone()
            .or(existing_plan.description.clone());
        let desired_amount = request.price.unwrap_or(existing_plan.price);

        if stripe_product_id.is_none() || stripe_price_id.is_none() {
            let (product_id, price_id) = self
                .stripe_service
                .create_plan_product_and_price(
                    plan_id,
                    creator_id,
                    &desired_name,
                    desired_description.as_deref(),
                    desired_amount,
            ...
            if request.name.is_some() || request.description.is_some() {
                if let Some(product_id) = &stripe_product_id {
                    self.stripe_service
                        .update_plan_product(
                            product_id,
            ...
            if request.price.is_some() {
                if let Some(product_id) = &stripe_product_id {
                    let new_price_id = self
                        ...
                        .await?;
                    stripe_price_id = Some(new_price_id);
                    price_updated = true;
                }
            }
        }

        if updates.is_empty() && !product_updated && !price_updated {
            return Err(AppError::Validation("没有提供更新字段".to_string()));
        }

        if product_updated {
            if let Some(product_id) = &stripe_product_id {
                updates.push(("stripe_product_id".to_string(), json!(product_id)));
            }
        }

        if price_updated {
            if let Some(price_id) = &stripe_price_id {
                updates.push(("stripe_price_id".to_string(), json!(price_id)));
            }
        }
    ...
    pub async fn get_creator_plans(
        ...
        let mut count_response = self
            .db
            .query_with_params(&count_query, query_params)
            .await?;
        ...
        let subscription_plans: Result<Vec<SubscriptionPlan>> = plans
            .into_iter()
    ...
    pub async fn create_subscription(
        ...
        request
            .validate()
            .map_err(|e| AppError::Validation(format!("订阅数据验证失败: {}", e)))?;
        ...
        if plan.creator_id == subscriber_id {
            return Err(AppError::BadRequest("无法订阅自己的计划".to_string()));
        }

        if self
            .check_existing_subscription(subscriber_id, &plan.creator_id)
            .await?
        {
            ...
            .filter(|pm| !pm.trim().is_empty())
        {
            ...
            let methods = self
                .stripe_service
                .list_payment_methods(subscriber_id)
                .await?;
        ...
        let stripe_subscription = self
            ...
            .create_subscription(
                ...
                CreateStripeSubscriptionRequest {
                    price_id: stripe_price_id.clone(),
        ...
        let started_at = stripe_subscription.current_period_start;
        let current_period_end = stripe_subscription.current_period_end;
        let status = Self::map_stripe_subscription_status(stripe_subscription.status.clone());
        ...
        let mut response = self
            ...
            CREATE subscription CONTENT {
                ...
                stripe_subscription_record_id: $stripe_subscription_record_id,
                created_at: time::now(),
                updated_at: time::now()
            }
        "#,
                json!({
                    ...
                    "stripe_subscription_record_id": stripe_subscription.id,
                }),
            )
            .await?;
        ...
        let subscription_details = self.build_subscription_details_sync(created, plan)?;
    ...
    pub async fn cancel_subscription(
        ...
        let mut subscription = self.get_subscription_with_plan(subscription_id).await?;
        ...
        if let Some(record_id) = subscription.stripe_subscription_record_id.as_deref() {
            self.stripe_service
                .cancel_subscription(record_id, at_period_end)
                .await?;
        }
        ...
        if !at_period_end {
            params["canceled_at"] = json!(now.to_rfc3339());
            params["current_period_end"] = json!(now.to_rfc3339());
        }

        let mut response = self.db.query_with_params(query, params).await?;
        let updated_records: Vec<Value> = response.take(0)?;
        ...
        subscription = self.build_subscription_details_sync(updated, plan)?;
    ...
    pub async fn check_subscription(
        ...
        debug!(
            "Checking subscription for user: {} to creator: {}",
        ...
        if let Some(result) = results.first() {
            ...
            let subscription_details =
                self.build_subscription_details_sync(result.clone(), plan)?;
    ...
    /// 获取创作者收益统计
    pub async fn get_creator_revenue(&self, creator_id: &str) -> Result<CreatorRevenue> {
        ...
        let stat = stats
            .first()
            .ok_or_else(|| AppError::NotFound("无法获取创作者统计".to_string()))?;
        ...
        // 获取最近订阅
        let recent_subscriptions = self
            .get_creator_recent_subscriptions(creator_id, 10)
            .await?;
    ...
    async fn verify_plan_ownership(&self, plan_id: &str, creator_id: &str) -> Result<()> {
        let query =
            "SELECT id FROM subscription_plan WHERE id = $plan_id AND creator_id = $creator_id";
        let mut response = self
            .db
            .query_with_params(
                query,
    ...
    async fn check_existing_subscription(
        &self,
        subscriber_id: &str,
        creator_id: &str,
    ) -> Result<bool> {
    ...
    fn build_subscription_details_sync(
        ...
        let creator = SubscriptionCreator {
            user_id: subscription_data["creator_id"]
                .as_str()
                .unwrap()
                .to_string(),
            ...
            display_name: subscription_data["display_name"]
                ...
                .unwrap_or("")
                .to_string(),
            avatar_url: subscription_data["avatar_url"]
                .as_str()
                .map(|s| s.to_string()),
        ...
        Ok(SubscriptionDetails {
            ...
            stripe_subscription_id: subscription_data["stripe_subscription_id"]
                .as_str()
                .map(|s| s.to_string()),
            stripe_subscription_record_id: subscription_data["stripe_subscription_record_id"]
                .as_str()
                .map(|s| s.to_string()),
    ...
    fn parse_subscription_plan(&self, plan_data: Value) -> Result<SubscriptionPlan> {
        Ok(SubscriptionPlan {
            ...
            stripe_product_id: plan_data["stripe_product_id"]
                .as_str()
                .map(|s| s.to_string()),
            stripe_price_id: plan_data["stripe_price_id"].as_str().map(|s| s.to_string()),
    ...
    fn map_stripe_subscription_status(status: StripeSubscriptionStatus) -> SubscriptionStatus {
        match status {
            ...
            StripeSubscriptionStatus::Incomplete | StripeSubscriptionStatus::IncompleteExpired => {
